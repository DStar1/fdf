...
// int	my_key_funct(int keycode, void *param)//, t_master *master)
// {
// 	printf("Key event %d\n", keycode);
// 	(void)param;
// 	return (0);
...
//////////////////
	...
	xlen = &lentmp; //gives a seg fault when I do *len = lentemp
	//ft_putstr(line);
	////twodstruct = (t_points**)malloc(sizeof(t_points*) * (*len) + 1); //change len to whatever length y is when you find out 
	twodstruct[0] = (t_points*)malloc(sizeof(t_points) * *xlen);
	znc((t_points**)twodstruct, ft_strsplit(line, ' '), xlen, 0);
	while ((get_next_line(fd, &line)))
	{
		twodstruct[i] = (t_points*)malloc(sizeof(t_points) * *xlen);
		znc((t_points**)twodstruct, ft_strsplit(line, ' '), xlen, i);
		i++;
	}
...
void draw(t_master *master, t_points **coords)
{
...
int main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		coords = points(av[1]);
		master.mlx = mlx_init();
		master.win = mlx_new_window(master.mlx, 800, 800, av[1]);
		draw(&master, coords);
		mlx_hook(master.win, 2, 0, my_key_funct, &master); //much faster //master or master->win as last var
		mlx_loop(master.mlx);
	}
